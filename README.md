...
- **Idle shell** → git branch if the cwd is in a repo (read from `.git/HEAD`, no subprocess) and the branch is not a default one (`main`/`master`/`trunk`/`develop`), else cwd basename. The home directory (`$HOME`) is named `home`, including in workspace project names and LLM context.
